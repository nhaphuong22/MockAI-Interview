...
export const updateProfile = async (req, res) => {
  try {
    const userId = req.user.id;
    const { fullName, phone, address, bio, avatarUrl, coverUrl, isLookingForJob, companyName, companyLogo, companyWebsite, companyDescription, companySize, companyIndustry, companyCity, companyAddress, contactEmail, contactPhone, contactPublic, linkedinUrl, githubUrl, portfolioUrl } = req.body;

    const result = await updateUserProfile(userId, { fullName, phone, address, bio, avatarUrl, coverUrl, isLookingForJob, companyName, companyLogo, companyWebsite, companyDescription, companySize, companyIndustry, companyCity, companyAddress, contactEmail, contactPhone, contactPublic, linkedinUrl, githubUrl, portfolioUrl });
    return sendResponse(res, 200, result);
  } catch (error) {
    if (error.message === 'User not found') {
      return sendError(res, 404, 'User not found');
