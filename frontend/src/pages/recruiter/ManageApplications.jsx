...
import { User, FileText, Calendar, Filter, Eye, ChevronDown, Check, Loader2, RefreshCw, Bookmark, Award, AlertCircle, Sparkles } from "lucide-react";
...
export function ManageApplications() {
  ...
  const getStatusBadgeStyles = (status) => {
    const defaultStyles = "bg-gray-100 text-gray-600";
    const statusMap = {
    ...
    return statusMap[status] || defaultStyles;
  };
  ...
  const getHrTagStyles = (tag) => {
    ...
    const tagMap = {
      ...
      REJECTED: "bg-red-100 text-red-755"
    };
    return tagMap[tag] || defaultStyles;
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-500 text-sm">Theo dõi, đánh giá hồ sơ và xem điểm phân tích phỏng vấn AI của ứng viên</p>
          </div>
          <button 
            onClick={() => refetch()} 
        ...
        {isLoadingApps ? (
          ...
          <div className="text-center py-24 bg-white rounded-3xl border border-gray-100 shadow-sm flex flex-col items-center justify-center">
            ...
            <p className="text-gray-500 text-sm max-w-sm">Chưa có hồ sơ nào nộp cho tin tuyển dụng đang lọc hoặc không tìm thấy kết quả.</p>
          </div>
        ) : (
            ...
            <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto z-50 animate-in zoom-in-95 duration-300 outline-none p-8">
              {selectedApplication && (
                <div className="space-y-6">
                  {/* Close button inside dialog is handled automatically by Radix or we can close via state */}
