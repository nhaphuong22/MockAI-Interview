  ...
  return (
    <div
      ...
      className={`bg-white rounded-xl p-6 border-2 cursor-pointer transition-all ${
        isSelected
          ? "border-[#0ea5e9] shadow-lg shadow-sky-50/50"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-4 mb-2">
            <div className="flex-1">
              <h3 className="text-lg font-semibold mb-1 text-gray-900 hover:text-[#0ea5e9] transition-colors truncate">
                {job.title}
              </h3>
              <p className="text-sm text-gray-500 font-medium">{job.company}</p>
            </div>
            <div className="flex items-center gap-3">
              <div className="px-3 py-1 bg-sky-50 text-[#0ea5e9] rounded-full text-xs font-bold border border-sky-100/50 shrink-0">
                {job.aiMatch}% phù hợp
              </div>
              <button
                onClick={(e) => {
          ...
          <div className="flex flex-wrap items-center gap-4 text-xs font-medium text-gray-500 mb-3">
            ...
            <div className="px-2 py-0.5 bg-blue-50 text-blue-700 rounded text-[10px] uppercase font-bold tracking-wider">
              {job.remote}
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {job.tags.map((tag) => (
              <span
                key={tag}
                className="px-2.5 py-0.5 bg-[#f0f9ff] text-[#0ea5e9] rounded-md text-xs font-semibold"
              >
                {tag}
              </span>
            ))}
