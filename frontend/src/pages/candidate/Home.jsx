...
export function Home() {
  return (
    <div className="bg-slate-50">
      ...
      <AuroraBackground className="min-h-[90vh] py-20 lg:py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full z-10 relative">
          ...
          >
            ...
            <div className="bg-white/60 backdrop-blur-xl border border-white/40 shadow-[0_8px_32px_rgba(14,165,233,0.1)] rounded-3xl p-3 flex flex-col md:flex-row gap-3 transition-all hover:shadow-[0_8px_32px_rgba(14,165,233,0.15)]">
              <div className="flex-1 flex items-center gap-3 px-5 py-3 bg-white/50 backdrop-blur-md rounded-2xl border border-white/50 focus-within:ring-2 focus-within:ring-[#0ea5e9]/20 focus-within:border-[#0ea5e9]/30 transition-all">
              ...
              <div className="flex items-center gap-3 px-5 py-3 bg-white/50 backdrop-blur-md rounded-2xl border border-white/50 focus-within:ring-2 focus-within:ring-[#0ea5e9]/20 transition-all md:w-64">
                <MapPin className="w-5 h-5 text-slate-400" />
                <select className="flex-1 bg-transparent outline-none text-slate-600 cursor-pointer">
            ...
            <div className="flex flex-wrap gap-2 mt-6 justify-center">
              {popularTags.map((tag, i) => (
                <motion.button
                  ...
                  className="px-4 py-1.5 bg-white/40 backdrop-blur-md border border-white/50 rounded-full text-sm text-slate-600 hover:border-[#0ea5e9]/50 hover:bg-[#f0f9ff]/80 hover:text-[#0ea5e9] transition-all"
                >
                  {tag}
                </motion.button>
              ))}
      ...
      <section className="py-12 bg-white/80 border-y border-slate-200/50 backdrop-blur-sm relative z-20 -mt-10 rounded-t-[3rem]">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {stats.map((stat, index) => {
              ...
              return (
                ...
                >
                  <div className="inline-flex items-center justify-center w-14 h-14 bg-sky-50 rounded-2xl mb-4 group-hover:scale-110 transition-transform duration-300">
                    <Icon className="w-6 h-6 text-[#0ea5e9]" />
                  </div>
                  <div className="text-3xl font-bold mb-1 text-slate-800">{stat.value}</div>
      ...
      <section className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-12">
            <div>
              <h2 className="text-3xl font-bold mb-2 text-slate-900 tracking-tight">Việc Làm Nổi Bật</h2>
              <p className="text-slate-500">Cơ hội việc làm tốt nhất dành cho bạn</p>
            </div>
            <Link
              to="/jobs"
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {featuredJobs.map((job, index) => (
              <motion.div
                ...
                key={job.id}
                className="bg-white rounded-2xl p-6 border border-slate-200/60 hover:border-[#0ea5e9]/30 hover:shadow-[0_8px_30px_rgb(14,165,233,0.12)] transition-all duration-300 cursor-pointer group relative overflow-hidden"
              >
                ...
                <div className="flex items-start gap-4 mb-5">
                  <div className="w-12 h-12 bg-gradient-to-br from-sky-50 to-white border border-sky-100 shadow-sm rounded-xl flex items-center justify-center text-2xl group-hover:scale-105 transition-transform">
                    {job.logo}
                  </div>
                  <div className="flex-1">
                    <h3 className="font-semibold text-slate-900 mb-1 group-hover:text-[#0ea5e9] transition-colors line-clamp-1">
                      {job.title}
                    </h3>
                    <p className="text-sm text-slate-500 line-clamp-1">{job.company}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {job.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-2.5 py-1 bg-slate-100/80 text-slate-600 rounded-lg text-xs font-medium"
                    >
                      {tag}
                    </span>
                  ))}
      ...
      <section className="py-24 bg-white relative overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-bl from-sky-50/50 to-transparent pointer-events-none rounded-bl-[100px]" />
        
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold mb-4 text-slate-900">Cách Thức Hoạt Động</h2>
            <p className="text-slate-500 max-w-2xl mx-auto">3 bước đơn giản để tìm việc mơ ước với sức mạnh của Trí tuệ nhân tạo</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-12 relative">
            ...
            {[
              ...
              <motion.div 
                ...
                className="text-center group"
              >
                <div className="inline-flex items-center justify-center w-24 h-24 bg-white border border-sky-100 shadow-[0_8px_30px_rgb(14,165,233,0.1)] rounded-3xl text-3xl font-bold text-[#0ea5e9] mb-6 group-hover:-translate-y-2 group-hover:shadow-[0_15px_40px_rgb(14,165,233,0.2)] transition-all duration-300">
