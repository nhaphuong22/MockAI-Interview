import { Bell, CheckCircle2, XCircle, MessageCircle, Briefcase, Star, Settings, Trash2, Search, Filter, Loader2 } from "lucide-react";
...
export function Notifications() {
  ...
  // Fetch danh sách thông báo thực tế
  const { data: response, isLoading, isError } = useQuery({
    queryKey: ["notifications"],
    queryFn: async () => {
      const res = await notificationApi.getNotifications();
      return res; // Axios interceptor bóc tách response.data
    }
  });
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] pt-20">
        ...
        <p className="text-gray-500 font-semibold text-sm">Đang tải thông báo của bạn...</p>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          {notifications.some(n => !n.isRead) && (
            <button 
              onClick={markAllRead}
              className="text-sm font-bold text-[#0ea5e9] hover:text-[#0284c7] transition-colors flex items-center gap-2 uppercase tracking-widest cursor-pointer"
            >
